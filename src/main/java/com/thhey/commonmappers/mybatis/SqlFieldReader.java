...
public class SqlFieldReader {
    ...
    /**
     * 获取表名，要求类上有 @TableAttribute 注解
     *
     * @param entity 实体类
     * @param <T>    实体类型
     * @return 表名
     */
    public static <T extends BaseEntity> String getTableName(T entity) throws BaseException {
        String tableName = tableNameMap.get(entity.getClass().getName());
        if (StringUtils.isNotEmpty(tableName)) {
            return tableName;
        }
        TableAttribute tableAttribute = entity.getClass().getAnnotation(TableAttribute.class);
        if (tableAttribute == null) {
            throw new BaseException("需要解析表名，但未找到 @TableAttribute 注解");
        }
        tableNameMap.put(entity.getClass().getName(), tableAttribute.name());
